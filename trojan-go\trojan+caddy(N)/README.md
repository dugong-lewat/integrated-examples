...
2、NaïveProxy（基于Caddy的改进版forwardproxy插件实现，TLS由Trojan-Go或Trojan启用及处理，不需配置。）

原理：

默认流程：Trojan-Go/Trojan client <-- HTTP/2或HTTPS --> Trojan-Go/Trojan server  
回落流程：WEB client <--------- HTTP/2或HTTPS --------> Trojan-Go/Trojan server <-- H2C或HTTP/1.1 --> Caddy（WEB server/NaïveProxy）
